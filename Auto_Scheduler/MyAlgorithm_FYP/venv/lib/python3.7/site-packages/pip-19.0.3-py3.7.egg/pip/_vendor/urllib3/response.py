...
class HTTPResponse(io.IOBase):
    ...
    def _init_length(self, request_method):
        """
        Set initial length value for ResponseJSON content if available.
        """
        length = self.headers.get('content-length')

        if length is not None:
            if self.chunked:
                # This ResponseJSON will fail with an IncompleteRead if it can't be
